...
<div style="padding: 64px 48px 48px 48px; background: #080808;">
    ...
    <div style="font-family: 'Cormorant Garamond', serif; font-size: 3.8rem; font-weight: 300;
        ...
        Discover Your<br>Property's True Value
    </div>
...
if predict_btn:
        ...
        <div style="display:flex; gap:48px; flex-wrap:wrap;">
            ...
            <div>
                ...
                <div style="font-family:'Cormorant Garamond',serif; font-size:1.8rem;
                    ...
                    AED {int(prediction*0.95):,} – {int(prediction*1.05):,}
                </div>
            </div>

            {"" if not has_comp else f'''
            <div style="width:1px; background:#2a2218;"></div>
            <div>
                <div style="font-size:0.62rem; letter-spacing:0.25em; text-transform:uppercase;
                            color:#4a4030; margin-bottom:8px;">vs Area Average</div>
                <div style="font-family:'Cormorant Garamond',serif; font-size:1.8rem;
                            font-weight:400; color:{"#a8d4a0" if delta_pct <= 0 else "#C9A84C"};">
                    {delta_pct:+.1f}%
                </div>
            </div>
            '''}
    ...
    if has_comp:
        ...
        for i, (_, row) in enumerate(top_similar.iterrows()):
            is_highlight = i == 0
            st.markdown(f"""
            <div style="background:{'linear-gradient(135deg,#1a1408,#100e06)' if is_highlight else '#0e0e0e'};
                        border:1px solid {'#C9A84C' if is_highlight else '#1e1a12'};
                        padding:28px 24px; position:relative;">
                {"<div style='position:absolute;top:16px;right:16px;font-size:0.55rem;letter-spacing:0.2em;color:#C9A84C;background:rgba(201,168,76,0.1);padding:4px 10px;border:1px solid rgba(201,168,76,0.3);'>HIGHEST</div>" if is_highlight else ""}
                <div style="font-family:'Cormorant Garamond',serif; font-size:1.6rem; font-weight:300;
                            color:{'#C9A84C' if is_highlight else '#f0e8d8'}; margin-bottom:6px;">
                ...
                <div style="font-size:0.65rem; color:#3a3020; letter-spacing:0.1em;">
                    {row['View']} View &nbsp;·&nbsp; {'Furnished' if row['Furnished']=='Yes' else 'Unfurnished'}
                </div>
            </div>
            """, unsafe_allow_html=True)
